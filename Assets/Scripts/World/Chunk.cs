...
public class Chunk : MonoBehaviour {
    ...
    void GrowPlants() {
        ...
        while (centerList.Count > 0) {
            ...
            temperature = tempNodes[Mathf.FloorToInt(center.x) + chunkSize / 2, Mathf.FloorToInt(center.y) + chunkSize / 2,
                Mathf.FloorToInt(center.z) + chunkSize / 2];
            prolificacy = prolNodes[Mathf.FloorToInt(center.x) + chunkSize / 2, Mathf.FloorToInt(center.y) + chunkSize / 2,
                Mathf.FloorToInt(center.z) + chunkSize / 2];

            //Debug.Log("temp " +  temperature);
            //Debug.Log("prol " +  prolificacy);
            Vector3 worldPos = transform.position + center;
            Vector3 noY = worldPos;
            noY.y = 0;
            ...
            if (structure && Vector3.Angle(normal, Vector3.up) < structure.GetComponent<NatureStructure>().maxAngle && floraValue >= 0) {
                GameObject plant = Instantiate(structure, worldPos,
                    Quaternion.FromToRotation(Vector3.up, (normal + Vector3.up / 1.5f)).normalized, transform);
    ...
    Vector3 CalcNormals(Vector3 a, Vector3 b, Vector3 c) {
        ...
        wrki = Mathf.Sqrt(Mathf.Pow(v1.y * v2.z - v1.z * v2.y, 2) + Mathf.Pow(v1.z * v2.x - v1.x * v2.z, 2) +
                          Mathf.Pow(v1.x * v2.y - v1.y * v2.x, 2));
