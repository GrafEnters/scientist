...
public class VillageGenerator : MonoBehaviour
{
	...
	public void GenerateVillage(Vector3 center)
	{
		...
		*/
		...
		GameObject elderObj = SpawnItem(elder, center + Vector3.right * 2 + Vector3.up * 30, elder.transform.rotation * Quaternion.Euler(Vector3.down * 90),villagerMatter);
		...
		signpost = SpawnItem(signPost, center + Vector3.forward * 5 + Vector3.left * 8, signPost.transform.rotation * Quaternion.Euler(Vector3.down * 210));
		signpost.SetActive(false);
		elderObj.GetComponent<Elder>().signPost = signpost;


		//Геолог и еже с ним
		GameObject geologObj = SpawnItem(geolog, center + Vector3.right * 2 + Vector3.up * 30, geolog.transform.rotation * Quaternion.Euler(Vector3.down * 90), villagerMatter);
		geologObj.SetActive(false);
		elderObj.GetComponent<Elder>().geolog = geologObj;
		geologObj.GetComponent<Geolog>().signPost = signpost;

		tmp = SpawnItem(pickaxe, center + new Vector3(2, 5, -2), pickaxe.transform.rotation * Quaternion.Euler(Vector3.down * 90), null, null, 0);
		tmp.SetActive(false);
		...
		//Кузнец и еже с ним
		GameObject smithObj = SpawnItem(blacksmith, center + Vector3.right * 2 + Vector3.up * 30, blacksmith.transform.rotation, villagerMatter);
		...
		tmp = SpawnItem(workbench, center + Vector3.left * 9 + Vector3.back * 4, workbench.transform.rotation * Quaternion.Euler(Vector3.down * -90));
		tmp.SetActive(false);
