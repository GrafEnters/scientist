...
public class ThingsGenerator : MonoBehaviour
{
	...
	public static GameObject GenerateThingPrefab(GameObject model, Matter main, Matter side, int needRarity = -1, MoleculeType molecule = MoleculeType.Fiber)
	{
		if ((main == null))
	...
	public static GameObject GenerateThing(GameObject model, Matter main, Matter side, int needRarity = -1, MoleculeType molecule = MoleculeType.Fiber)
	{
		if ((main == null))
			main = GetNextMatter(needRarity, molecule);
	...
	public void GenerateNatureStructures()
	{
		...
		while (index < mattersQueue.Count * 2)
		{
			...
			if (mainMatter.moleculeType == MoleculeType.Crystal)
				SStructures[index] = GenerateThingPrefab(NatureRocks[Random.Range(0, NatureRocks.Length)], mainMatter, null, mainMatter.Rarity, MoleculeType.Crystal);
			else
				SStructures[index] = GenerateThingPrefab(NaturePlants[Random.Range(0, NatureRocks.Length)], mainMatter, null, mainMatter.Rarity, MoleculeType.Fiber);
			index++;
		}
