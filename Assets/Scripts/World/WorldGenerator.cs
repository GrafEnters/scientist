...
public class WorldGenerator : MonoBehaviour {
    private int ChunkSize => ConfigsProvider.Instance.MainGameConfig.ChunkSize;

    [Range(1, 15)]
    public int islandRadiusInChunks;

    private int PolygonSize => ConfigsProvider.Instance.MainGameConfig.PolygonSize;
    ...
    public string seed;
    private int ViewDistanceInChunks => ConfigsProvider.Instance.MainGameConfig.ViewDistanceInChunks;
    ...
    void SpawnChunk(Vector3 coord) {
        Chunk chunk = Instantiate(ChunkPrefab, new Vector3(coord.x, coord.y, coord.z) * ChunkSize + Vector3.up * ChunkSize / 2,
        ...
        if (chunk.height < WorldNoise.IslandTerrainNoise(coord * ChunkSize, ChunkSize) - ChunkSize * 2) {
            chunk.biome = new Biome(biomes[^1]);
        } else
            foreach (Biome t in biomes) {
                if (chunk.height >= t.minHeight)
                    chunk.biome = new Biome(t);
                if (chunk.height < t.minHeight)
                    break;
            }
...
public class LandNoiseGenerator {
    ...
    public int Noise(Vector3 point, int chunkSize) {
        ...
        if (Mathf.Abs(point.x) < chunkSize * 1.5f && Mathf.Abs(point.z) < chunkSize * 1.5f)
            ...
            return SNG.coherentNoise(new Vector3(point.x, height, point.z), octaves, multiplier, dump, sharpness, persistance) <= (densityEdge)
                ? 1
                : 0;
    ...
    public float IslandTerrainNoise(Vector3 point, int chunkSize) {
        ...
        if (noY.magnitude < FlatspawnRadius)
            ...
            height = SNG.coherentNoise(new Vector3(point.x, 10000, point.z), octaves, multiplier * 10, dump, sharpness, persistance) -
                     negativedump; // -            Mathf.Pow(noY.magnitude - Radius * chunkSize, 1f / 1.8f) * 5 ;
