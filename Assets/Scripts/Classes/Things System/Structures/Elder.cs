...
public class Elder : Villager
{
	...
	protected override Task GenerateTask()
	{
		return new Task
		{
			...
			text = "Раз я здесь самый первый, то возьму на себя роль старейшины. А все старейшины любят когда в их деревне покой, и всякие падающие с неба люди не кричат почём зря. Впредь, если кому-то нужна твоя помощь - он оставит письмо. Принеси мне это письмо, кстати.\n- ПКМ на меня с письмом в руке завершит задание",
			name = "Карл",
			villager = this
		};
	}
	...
	void BringLogsCheck()
	{
		...
		if (have == logNeeded)
		{
			...
			TalkManager.Say("Вот тебе ящик, с ним будет поудобнее. Клавишей Е сможешь достать из за спины, а нажав 1, повесишь инструмент на крючок. ");
			firstMet++;
			SpawnGeolog();
