...
public class Geolog : Villager
{
	...
	void BringBerriesCheck()
	{
		...
		if (have == berryNeeded)
		{
			...
			TalkManager.Say("Великолепно! Вот тебе моя кирка, с ней ты сможешь добыть любой минерал. Иди поищи чего интересного, а я пока тут всё подготовлю.");
			SpawnStation();
			signPost.GetComponent<SignPost>().EnqueTask(GenerateTask());
			firstMet++;
		...
		switch (rnd)
		{
			...
			case 2:
				return new Task
				{
					...
					villager = this,
					text = "Вижу импровизированную полку ты уже заполнил. Я готова потрудиться и сделать новую, но взамен ты должен принести мне кирку. Можешь вернуть мою, можешь у Ефима сделать новую. \n- Как сделаешь кирку, просто брось её рядом со мной, я пойму что она для меня",
					name = "Клара"
				};
			case 3:
				return new Task
	...
	public override bool Interact(GameObject handObj = null)
	{
		...
		{
			TalkManager.Say("Покорять новые миры я не гожусь, но мне есть что тебе предложить. Но пока я не подкреплюсь, я не смогу думать ни о чём кроме еды.");
			firstMet++;
		}
		else if (firstMet == 2)
		{
