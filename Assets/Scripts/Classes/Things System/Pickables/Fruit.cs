...
public class Fruit : Pickable, IEdible {
    ...
    public override void Init(int id, Matter main, Matter side) {
        ...
        edible = new Edible {
            ...
            dragEffects = Random.Range(0, 4) != 0 ? Edible.DragEffects.None :
                Random.Range(0, 2) == 0 ? Edible.DragEffects.Cafein : Edible.DragEffects.Cocaine //Мдэ, надо переписать
        };
    }

    public void GetDadandSon(GameObject dad) {
        parentStructure = dad;
        SeedPrefab = ThingsGenerator.GenerateThingPrefab(SeedModel, parentStructure.GetComponent<Thing>().mainM,
            parentStructure.GetComponent<Thing>().sideM);
