    ...
    void Drag() {
        if (draggedObject.GetComponent<Pickable>().thingName == "crate")
            draggedRB.linearVelocity =
                ((Camera.main.transform.position + Camera.main.transform.forward * (distance + 2) - draggedObject.transform.position) * smooth);
        else
            draggedRB.linearVelocity =
                ((Camera.main.transform.position + Camera.main.transform.forward * distance - draggedObject.transform.position) * smooth);

        draggedObject.transform.rotation = Quaternion.identity;
        Quaternion target = Quaternion.Euler(rotationVctr);
        draggedObject.transform.localRotation =
            target; //Quaternion.Slerp(draggedObject.transform.localRotation, target, Time.deltaTime * smoothRot);
    }
